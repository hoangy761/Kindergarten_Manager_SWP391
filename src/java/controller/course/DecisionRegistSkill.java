 ...
 * @author ASUS-PRO
 */
public class DecisionRegistSkill extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (status.equals("Accept")) {
            SkillListDB.UpdateStatus(idChild, status);
            SkillList list = SkillListDB.getAllChildByIdChild(idChild, status);

            if (SkillListDB.getTotalSeat(list.getIdSkill()) > 0) {
                SkillListDB.UpdateSubTotalSeat(list.getIdSkill());
                if (TuitionV2DB.checkTuition(idChild) == true) {
                    SkillList list1 = SkillListDB.getAllChildByIdChild2(idChild);
                    TuitionV2 v1 = TuitionV2DB.getTuition(idChild);
                    TuitionV2DB.addListFeesV2(Validator.createID(), list.getIdSkill(), idChild, list1.getTuition(), "No", v1.getIdTuition());
                    ...
                    TuitionV2DB.addListFeesV2(Validator.createID(), list.getIdSkill(), idChild, tuitionV2.getTuition(), "No", tuitionV2.getIdTuition());          
                    response.sendRedirect("class-registration-list");
                }
            ...
            SkillList list = SkillListDB.getAllChildByIdChild2(idChild);

            if (SkillListDB.delete(list.getIdSkill())) {
                response.sendRedirect("class-registration-list");
            } else {
                request.getRequestDispatcher("admin.jsp").forward(request, response);
            }
